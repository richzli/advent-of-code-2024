...
parseLine s = (read left, parseNumbers right)
    where left:right:_ = strip <$> split (==':') s

main :: IO ()
main = do
    input <- parseLines <$> getContents
    print $ sum $ fmap fst $ filter (\(target, l) -> target `elem` (foldl1' (\x y -> [(+), (*)] <*> x <*> y) (pure <$> l))) (parseLine <$> input)