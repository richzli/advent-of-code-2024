...
checksum limit i ((fid, len):fs) = if limit - i <= len
    then max fid 0 * (i * (limit - i) + sumTo (limit - i))
    else max fid 0 * (i * len + sumTo len) + checksum limit (i + len) fs
checksum _ _ [] = 0

compact :: (Int, Int) -> [(Int, Int)] -> [(Int, Int)]
compact f'@(fid', len') (f@(fid, len):fs)
    | fid /= -1  = f : compact f' fs
    | len < len' = f : compact f' fs
    | otherwise  = f' : (fid, len - len') : fs
compact _ [] = []

clean :: Set.Set Int -> [(Int, Int)] -> [(Int, Int)]
clean s (f@(fid, len):fs)
    | len == 0     = clean s fs
    | fid == -1    = f : clean s fs
    | fid `elem` s = (-1, len) : clean s fs
    | otherwise    = f : clean (Set.insert fid s) fs
clean _ [] = []

main :: IO ()
main = do
    input <- fmap (read . singleton) . strip <$> getContents
    let (files, frees) = separate input
    let ids = zip [0..] files
    let chunks = interleave ids $ (-1,) <$> frees

    print $ checksum (sum input) 0 $ clean Set.empty $ foldr compact chunks ids
