...
trek :: Map.Map Point Int -> Map.Map Point (Set.Set Point) -> Map.Map Point (Set.Set Point)
trek g m = Map.foldrWithKey (\k a b -> foldl' (\m' p -> Map.insertWith (Set.union) p a m') b $ filter (\p -> Map.lookup p g == ((+1) <$> Map.lookup k g)) $ addPoint k <$> d4) Map.empty m

main :: IO ()
main = do
    input <- parseLines <$> getContents
    let grid = Map.fromList $ enumerate2d $ fmap ((read::String->Int) . singleton) <$> input
    let trailheads = Map.mapWithKey (const . Set.singleton) $ Map.filter (==0) grid

    print $ Map.foldr ((+) . Set.size) 0 $ iterate (trek grid) trailheads !! 9