...
halve :: [a] -> [[a]]
halve l = (<*>) [fst, snd] . pure $ splitAt ((length l + 1) `div` 2) l

blink :: Int -> [Int]
blink n
    | n == 0                         = [1]
    | even (length (show n) `mod` 2) = read <$> halve (show n)
    | otherwise                      = [n * 2024]

main :: IO ()
main = do
    input <- parseNumbers . strip <$> getContents
    let stones = foldr (\k m -> Map.insertWith (+) k 1 m) Map.empty input
    print $ Map.foldr (+) 0 $ iterate (Map.foldrWithKey (\k a b -> foldr (\j m -> Map.insertWith (+) j a m) b $ blink k) Map.empty) stones !! 75