...
remove :: [a] -> [[a]]
remove [] = []
remove (x:xs) = xs : (((:) x) <$> remove xs)

main :: IO ()
main = do
    input <- getContents
    let reports = parseNumbers <$> parseLines input
    let removed = remove <$> reports
    let c = map (\r -> uncurry (-) <$> zip r (tail r)) <$> uncurry (:) <$> zip reports removed
    print $ length $ filter (any (\d -> ((all (< 0) d) || (all (> 0) d)) && (all (<= 3) $ abs <$> d))) c