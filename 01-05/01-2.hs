...
main = do
    input <- getContents
    let left:right:_ = transpose $ parseNumbers <$> parseLines input
    let cnt = foldl (\m v -> Map.insertWith (+) v 1 m) Map.empty right
    print $ sum $ (\v -> v * Map.findWithDefault 0 v cnt) <$> left