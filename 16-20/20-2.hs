...
jumps :: [[Point]]
jumps = iterate (nub . (<*>) (addPoint <$> d4)) [(0, 0)]

jump20 :: [(Point, Int)]
jump20 = Map.toList $ Map.fromList $ reverse $ concat $ zipWith (fmap . flip (,)) [0..] $ take 21 jumps
...
cheats dists p = length $ filter (\(d', l) -> d-d' >= 100+l) $ mapMaybe (\(f, s) -> (,s) <$> Map.lookup (addPoint p f) dists) jump20
