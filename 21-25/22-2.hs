...
diff :: [Int] -> [(Int, Int)]
diff l = zip (zipWith (-) (tail l) l) (tail l)

hash :: [Int] -> Int
hash [] = 0
hash (x:xs) = x + 20 * hash xs

match :: [(Int, Int)] -> Map.Map Int Int
match l
    | length l < 4 = Map.empty
    | otherwise    = Map.insert (hash seq) (snd $ (l !! 3)) (match $ tail l)
    where
        seq = fst <$> take 4 l
...
main = do
    ...
    let monkeys = diff . fmap (`mod` 10) . take 2001 . iterate evolve <$> input

    print $ Map.foldr max 0 $ Map.unionsWith (+) $ fmap match monkeys