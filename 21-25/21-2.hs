...
by2s :: String -> Map.Map String Int
by2s s = foldr (($ 1) . Map.insertWith (+)) Map.empty $ zipWith (:) ('A':s) (singleton <$> s)

solve :: (Char -> Char -> String) -> Map.Map String Int -> Map.Map String Int
solve f m = Map.foldr (Map.unionWith (+)) Map.empty $ Map.mapWithKey (\[c1, c2] v -> Map.map (*v) (by2s $ f c1 c2)) m 
...
main = do
    ...
    --print $ solve go2 . solve go1 . by2s <$> ["7"]
    print $ sum $ zipWith (*) ids $ Map.foldr (+) 0 . (!! 25) . iterate (solve go2) . solve go1 . by2s <$> input