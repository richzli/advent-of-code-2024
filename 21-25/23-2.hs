...
cliqueAdd :: Map.Map String (Set.Set String) -> Set.Set (Set.Set String) -> Set.Set (Set.Set String)
cliqueAdd m prev = Set.unions $ (\s -> let
        l = Set.toList s
    in
        Set.map (($ s) . Set.insert) $ Set.filter (all (inGraph m) . ($ l) . fmap . (,)) $ Set.unions $ mapMaybe (($ m) . Map.lookup) l
    ) <$> Set.toList prev
...
main = do
    ...
    let tris = Set.fromList $ concat $ zipWith (fmap . Set.insert) computers $ fmap (fmap (\(u, v) -> Set.fromList [u, v]) . filter (inGraph g) . pairs) $ ($ g) . Map.findWithDefault Set.empty <$> computers
    putStrLn $ intercalate "," $ Set.toAscList $ head $ Set.toList $ last $ takeWhile (not . Set.null) $ iterate (cliqueAdd g) $ Set.fromList $ Set.singleton <$> computers